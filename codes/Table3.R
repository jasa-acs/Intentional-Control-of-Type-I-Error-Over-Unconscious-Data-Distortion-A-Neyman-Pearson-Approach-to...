...
for (i in 1:repl){
  ...
  A_train   = A_shuffle[1:floor(n_1/rA),]
  B_train   = B_shuffle[1:floor(n_2/rB),]
  ...
  y_train = c(y_1[1:floor(n_1/rA)],y_2[1:floor(n_2/rB)])
  X_test    = rbind(A_shuffle[(floor(n_1/rA)+1):n_1,],B_shuffle[(floor(n_2/rB)+1):n_2,])
  X_testn   = posterior(object,X_test)$topics
  
  y_test    = c(y_1[(floor(n_1/rA)+1):n_1],y_2[(floor(n_2/rB)+1):n_2])
  #start.Umbrella <- Sys.time()
  result <- umbrellaAlg(X_train,y_train,X_testn,y_test,0.3,0.2)
...
save(Isvmr,IIsvmr,Isvm,IIsvm,Ir,IIr,I,II,Inbr,IInbr,Inb,IInb,Irfr,IIrfr,Irf,IIrf,Idsdar,IIdsdar,Idsda,IIdsda,file="Table3.RData")
